...
//#pragma HLS UNROLL
                    if ((n + Tn > N && i + n + 0 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        ...
                        buf[i + 0][j - r * S + P][k - c * S + P] = *(in_data_1 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 1 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 1][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 1][j - r * S + P][k - c * S + P] = *(in_data_2 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 2 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 2][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 2][j - r * S + P][k - c * S + P] = *(in_data_3 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 3 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 3][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 3][j - r * S + P][k - c * S + P] = *(in_data_4 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 4 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 4][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 4][j - r * S + P][k - c * S + P] = *(in_data_5 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 5 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 5][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 5][j - r * S + P][k - c * S + P] = *(in_data_6 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 6 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 6][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 6][j - r * S + P][k - c * S + P] = *(in_data_7 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
                    if ((n + Tn > N && i + n + 7 >= N ) || j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        buf[i + 7][j - r * S + P][k - c * S + P] = T(0);
                    }else {
                        buf[i + 7][j - r * S + P][k - c * S + P] = *(in_data_8 + (i+n)/8 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                    }
				}
			}
		}
    ...
    void output_res(G out_buf[][Tr][Tc],G *out_data_1,G *out_data_2,G *out_data_3,G *out_data_4,G *out_data_5,G *out_data_6,G *out_data_7,G *out_data_8, int n, int r, int c, int N, int R, int C, bool act){
        for (int j = r; j < r + Tr && j < R; j++) {
            for (int k = c; k < c + Tc && k < C; k++) {
                for (int i = 0; i < Tn && i < N - n; i += 8) {
#pragma HLS PIPELINE
                        if (act) {
//                        	if (i + 0 < N - n)
                            	*(out_data_1 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 0][j - r][k - c]);
//                        	if (i + 1 < N - n)
                            	*(out_data_2 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 1][j - r][k - c]);
//                        	if (i + 2 < N - n)
                            	*(out_data_3 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 2][j - r][k - c]);
//                        	if (i + 3 < N - n)
                            	*(out_data_4 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 3][j - r][k - c]);
//                        	if (i + 4 < N - n)
                            	*(out_data_5 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 4][j - r][k - c]);
//                        	if (i + 5 < N - n)
                            	*(out_data_6 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 5][j - r][k - c]);
//                        	if (i + 6 < N - n)
                            	*(out_data_7 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 6][j - r][k - c]);
//                        	if (i + 7 < N - n)
                            	*(out_data_8 + ((i+n)/8) * R * C + j * C + k) = relu(out_buf[i + 7][j - r][k - c]);
                        }
